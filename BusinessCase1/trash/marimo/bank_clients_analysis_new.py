...
def __(RESULTS, json, mo, np, pd):
    _missing = []
    for _f in ["df_cluster.parquet", "distance_matrix_w.npy", "metrics.json",
               "labels_k3.npy", "umap2d_w.npy", "umap3d_w.npy"]:
        if not (RESULTS / _f).exists():
            _missing.append(_f)
    ...
    distance_matrix_w = np.load(RESULTS / "distance_matrix_w.npy")
    distance_matrix_w_w = np.load(RESULTS / "distance_matrix_w_w.npy")

    with open(RESULTS / "metrics.json") as _f:
        _raw_metrics = json.load(_f)
    metrics_dict_w = {int(k): v for k, v in _raw_metrics.items()}

    with open(RESULTS / "metrics_w.json") as _f:
        _raw_metrics_w = json.load(_f)
    metrics_dict_w_w = {int(k): v for k, v in _raw_metrics_w.items()}

    K_RANGE   = sorted(metrics_dict_w.keys())
    labels_by_k_w = {k: np.load(RESULTS / f"labels_k{k}.npy") for k in K_RANGE}
    labels_by_k_w_w = {k: np.load(RESULTS / f"labels_k{k}_w.npy") for k in K_RANGE}
    
    umap2d_w    = np.load(RESULTS / "umap2d_w.npy")
    umap3d_w    = np.load(RESULTS / "umap3d_w.npy")
    umap2d_w_w  = np.load(RESULTS / "umap2d_w_w.npy")
    umap3d_w_w  = np.load(RESULTS / "umap3d_w_w.npy")

    mo.vstack([
        mo.callout(mo.md("Precomputed results loaded successfully."), kind="success"),
        mo.md(
            f"- Dataset: **{df_cluster.shape[0]:,} clients** x {df_cluster.shape[1]} features  \n"
            f"- Distance matrix: **{distance_matrix_w.shape[0]:,} x {distance_matrix_w.shape[0]:,}** "
            f"({distance_matrix_w.nbytes / 1e6:.0f} MB)  \n"
            f"- Metrics available for k: **{K_RANGE}**"
        ),
    ])
    return (
