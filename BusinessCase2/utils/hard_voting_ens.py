...
Identical component set to ``soft_voting_ens.py``; differs only in the
aggregation rule (majority vote vs probability averaging).

Hard voting discards probability information before aggregation and is
generally inferior to soft voting for well-calibrated models. It is
...
PICKLE_DIR: Path = (
    Path(__file__).parent.parent / "data" / "pickled_files" / "hard_voting_ens"
)
...
def _make_model(
    ...
    """
    Build the hard-voting ensemble with per-estimator pipelines.
    ...
    class_weight : ``'balanced'`` or None
        Passed to LR and SVM.

    Returns
    -------
    VotingClassifier
        Unfitted hard-voting ensemble.
    """
    lr_pipe = Pipeline([
...
def run_for_target(df, target_col: str) -> dict:
    """
    Train and evaluate the hard-voting ensemble for one binary target.
    ...
    # Hard voting with predict_proba available (SVM has probability=True)
    try:
...
def main() -> None:
    """Train, evaluate, and pickle the hard-voting ensemble for all targets."""
    df = load_data()
