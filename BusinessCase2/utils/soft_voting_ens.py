...
(Section 3.8 of the paper — NOT RF+XGB+LR as in the original notebook.)
...
Because their errors arise from different mechanisms they are partially
uncorrelated; averaging calibrated probabilities reduces total generalisation
error relative to any individual component.

Soft voting is preferred over hard voting because it uses full probability
information before discretisation, producing better-calibrated aggregate
predictions (Section 3.8).

Per-estimator pipeline encapsulation (Section 3.3):
- LR pipeline  : StandardScaler → LogisticRegression(L1, liblinear)
- XGB pipeline : (no scaler) → CalibratedXGBoost
- SVM pipeline : MinMaxScaler → SVC(RBF, probability=True)

The top-level split uses ``split_data`` (no global scaling); each estimator
handles its own preprocessing inside its Pipeline, preventing leakage during
cross-validation.
...
PICKLE_DIR: Path = (
    Path(__file__).parent.parent / "data" / "pickled_files" / "soft_voting_ens"
)
...
def _make_model(
    ...
    ----------
    spw : float
        ``scale_pos_weight`` for XGBoost (pass neg/pos ratio for imbalanced
        targets, 1.0 for balanced ones).
    class_weight : ``'balanced'`` or None
        Passed to LR and SVM (apply for IncomeInvestment only).

    Returns
    -------
    VotingClassifier
        Unfitted soft-voting ensemble.
    """
    # LR: StandardScaler + L1 regularisation (Section 3.3)
    lr_pipe = Pipeline([
    ...
    # XGBoost: no global scaling (tree-invariant); calibrated via isotonic
    ...
    # SVM (RBF): MinMaxScaler, probability=True via Platt scaling (Section 3.3)
    svm_pipe = Pipeline([
        ("scaler", MinMaxScaler()),
        ("clf", SVC(
            kernel="rbf",
...
def run_for_target(df, target_col: str) -> dict:
    """
    Train and evaluate the soft-voting ensemble for one binary target.

    Uses F_E for all components. scale_pos_weight and class_weight applied
    for IncomeInvestment only. 10-fold stratified CV.
    ...
    -------
    dict
        Canonical result dictionary with ``'threshold_info'`` and
        ``'brier_score'`` keys.
    """
    ...
    logger.info("  [F_E] Brier score: %.4f", brier)
    ...
    # Ablation: same ensemble on F_B
    X_base = build_baseline_features(df)
    X_tr_b, X_te_b, _, _ = split_data(X_base, y)
    cv_raw_b = compute_cv_metrics(_make_model(spw, cw), X_tr_b, y_tr)
    ...
    return make_result_dict(
        ...
        ablation={
            "engineered": {
                "cv_metrics_raw": cv_raw,
                "cv_metrics_summary": summarise_cv(cv_raw),
                "test_metrics": test_m,
            },
            "baseline": {
...
def main() -> None:
    """Train, evaluate, and pickle the soft-voting ensemble for all targets."""
    df = load_data()
